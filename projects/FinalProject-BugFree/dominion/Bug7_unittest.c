...
//TEST 1: EXECUTE REFACTORED TRIBUTE FUNCTION
        printf("---------------------------------------\n");
        ...
        //Fill currentPlayer deck with 5 cards to draw from (deckCount to 5) so player may draw
        for(i = 0; i < G.deckCount[p]; i++)
        {
            if(i == 0)  //Fill with copper #4
            {
                G.deck[p][i] = 4;
            }
            if(i == 1)  //Fill with Silver #5
            {
                G.deck[p][i] = 5;
            }
            if(i == 2)  //Fill with Gold #6
            {
                G.deck[p][i] = 6;
            }
            if(i == 3)  //Fill with Mine #11
            {
                G.deck[p][i] = 11;
            }
            if(i == 4)  //Fill with Estate #1
            {
                G.deck[p][i] = 1;
            }
        }
        ...
        for(i = 1; i < G.numPlayers; i++)
        {
            ...
            {
                //Use action card -- village #14 to test action
...
//Call cardEffect with tribute - if card is treasure, +2 coins
         if(cardEffect(19, 0, 0, 0, &G, 1, 2) == 0) 
        {
 ...
 //TEST 4: PLAYER REVEALS AND DISCARDS 2 CARDS 
        ...
        for(i = 1; i < G.numPlayers; i++)
        {
            for(j = 0; j < G.deckCount[i]; j++)
            {
                if((G.deck[i][j] == tributeRevealedCards[0]) || (G.deck[i][j] == tributeRevealedCards[1]))
                {
                    printf("G.deck[i][j] %d\n", G.deck[i][j]);
                    cardExists = 1;
                }
        ...
        if(cardExists == 1) 
        {
            printf("    FAIL: Tribute revealed card %s is still in deck\n", card_name);
        }
        else
        {
